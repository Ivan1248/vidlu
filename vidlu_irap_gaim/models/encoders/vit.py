...
# Files required to load a DiNOv2 model locally
_DINOV2_FILES = ["config.json", "model.safetensors"]
...
class ViTEncoder(nn.Module):
    ...
    def forward(self, frame: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.patch_size is None:
            raise RuntimeError("ViTEncoder requires patch_size to compute spatial layout.")
        ...
        feature_map = patch_tokens.transpose(1, 2).reshape(B, dim, grid_h, grid_w)
