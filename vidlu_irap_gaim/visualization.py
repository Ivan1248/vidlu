            ...
            if x is not None:
                ...
                with open(os.path.join(self.debug_dir, "input_stats.txt"), "w") as f:
                    ...
                    for i in range(min(imgs.shape[0], 8)):
                        ...
                        f.write(f"Image {i} - Min: {img_tensor.min().item()}, Max: {img_tensor.max().item()}, Mean: {img_tensor.mean().item()}, Std: {img_tensor.std().item()}\n")

                # BIH mean and std (from config.json)
                mean = torch.tensor([0.53354913, 0.52727484, 0.48752149]).view(3, 1, 1)
                std = torch.tensor([0.20401913, 0.20417478, 0.25402164]).view(3, 1, 1)
