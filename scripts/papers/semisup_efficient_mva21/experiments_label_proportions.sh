...
for s in {0..0}; do
  ...
  do
    ...
    python run.py train "train,test:Cityscapes(downsampling=2){train,val}:(folds(d[0].permute(${seed}),${divisor})[0],d[1])" "standardize(cityscapes_mo)" "SwiftNet,backbone_f=t(depth=18)" "tc.swiftnet_cityscapes_halfres,lr_scheduler_f=lr.QuarterCosLR,epoch_count=${epoch_count},batch_size=8" --params "resnet:backbone->backbone.backbone:resnet18" -r ?
  done
done
...
for s in {0..4}; do
  ...
  do
    let epoch_count=divisor*200  # 1600, 800, 400, 200
    printf "\n${name}: seed=$seed, 1/$divisor labeled examples, $epoch_count epochs\n\n"
    python run.py train "train,train_u,test:Cityscapes(downsampling=2){train,val}:(folds(d[0].permute(${seed}),${divisor})[0],d[0],d[1])" "standardize(cityscapes_mo)" "SwiftNet,backbone_f=t(depth=18)" "tc.swiftnet_cityscapes_halfres,tc.semisup_cons_phtps20_seg,train_step=ts.MeanTeacherStep(alpha=0.5),lr_scheduler_f=lr.QuarterCosLR,epoch_count=${epoch_count},batch_size=[8,8]" --params "resnet:backbone->backbone.backbone:resnet18" -r ?
  done
done
...
for s in {0..4}; do
  ...
  do
    let epoch_count=divisor*200  # 1600, 800, 400, 200
    printf "\n${name}: seed=$seed, 1/$divisor labeled examples, $epoch_count epochs\n\n"
    python run.py train "train,train_u,test:Cityscapes(downsampling=2){train,val}:(folds(d[0].permute(${seed}),${divisor})[0],d[0],d[1])" "standardize(cityscapes_mo)" "SwiftNet,backbone_f=t(depth=18)" "tc.swiftnet_cityscapes_halfres,tc.semisup_cons_cutmix,train_step=ts.MeanTeacherStep(alpha=0.5),lr_scheduler_f=lr.QuarterCosLR,epoch_count=${epoch_count},batch_size=[8,8]" --params "resnet:backbone->backbone.backbone:resnet18" -r ?
  done
done
...
for s in {0..4}; do
  ...
  do
    let epoch_count=divisor*200  # 1600, 800, 400, 200
    printf "\n${name}: seed=$seed, 1/$divisor labeled examples, $epoch_count epochs\n\n"
    python run.py train "train,train_u,test:Cityscapes(downsampling=2){train,val}:(folds(d[0].permute(${seed}),${divisor})[0],d[0],d[1])" "standardize(cityscapes_mo)" "SwiftNet,backbone_f=t(depth=18)" "tc.swiftnet_cityscapes_halfres,tc.semisup_cons_cutmix,train_step=ts.MeanTeacherStep(loss_cons=partial(losses.conf_thresh_probs_sqr_l2_dist_ll,conf_thresh=0.97)),lr_scheduler_f=lr.QuarterCosLR,epoch_count=${epoch_count},batch_size=[8,8]" --params "resnet:backbone->backbone.backbone:resnet18" -r ?
  done
done
