...
simp_1w_ps="ts.SemisupVATTrainStep(alpha=0.5)" # P -> C
simp_1w_pt="ts.SemisupVATTrainStep(alpha=0.5,rev_cons=True,block_grad_on_pert=True,block_grad_on_clean=False)"  # C -> P
simp_2w_p1="ts.SemisupVATTrainStep(alpha=0.5,rev_cons=True,block_grad_on_pert=False,block_grad_on_clean=False)" # C <-> P
simp_1w_p2="ts.SemisupVATTrainStep(alpha=0.5,pert_both=True)" # P -> P
...
mt_1w_p2="ts.MeanTeacherStep(alpha=0.5,pert_both=True)"

for s in {0..4}; do
  let seed=start_seed+s

  printf "\n${name}: simple-CS-1/4 supervised: seed=$seed, 800 epochs\n\n"
  python run.py train "train,test:Cityscapes(downsampling=2){train,val}:(folds(d[0].permute(${seed}),4)[0],d[1])" "standardize(cityscapes_mo)" "SwiftNet,backbone_f=t(depth=18)" "tc.swiftnet_cityscapes_halfres,lr_scheduler_f=lr.QuarterCosLR,epoch_count=800,batch_size=8" --params "resnet:backbone->backbone.backbone:resnet18" --resume_or_start
...
simp_2w_p1="ts.SemisupVATTrainStep(rev_cons=True,block_grad_on_pert=False,block_grad_on_clean=False)"
simp_1w_p2="ts.SemisupVATTrainStep(pert_both=True)"

mt_1w_ps="ts.MeanTeacherStep()"
mt_1w_pt="ts.MeanTeacherStep(rev_cons=True,block_grad_on_pert=True,block_grad_on_clean=False)"
mt_1w_p2="ts.MeanTeacherStep(pert_both=True)"
