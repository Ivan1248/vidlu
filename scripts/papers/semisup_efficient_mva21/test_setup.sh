...
printf "\n\nSwiftNet-RN18, half-resolution Cityscapes\n"
python run.py train "train,test:Cityscapes(downsampling=2){train,val}:(folds(d[0].permute(53),4)[0],d[1])" "standardize(cityscapes_mo)" "SwiftNet,backbone_f=t(depth=18)" "tc.swiftnet_cityscapes_halfres,lr_scheduler_f=lr.QuarterCosLR,epoch_count=${epoch_count},batch_size=8" --params "resnet:backbone->backbone.backbone:resnet18" --no_train_eval -r ?
...
# WRN-28-2, CIFAR-10
printf "\n\nWRN-28-2, CIFAR-10\n"
python run.py train "train,test:Cifar10{trainval,test}:(rotating_labels(d[0])[:4000],d[1])" id "WRN,backbone_f=t(depth=28,width_factor=2,small_input=True)" "tc.wrn_cifar,epoch_count=${epoch_count},batch_size=128,eval_batch_size=640" --no_train_eval -r ?