...
printf "\n\nSwiftNet-RN18, half-resolution Cityscapes\n"
python run.py train "train,test:Cityscapes(downsampling=2){train,val}:(folds(d[0].permute(53),4)[0],d[1])" "standardize(cityscapes_mo)" "SwiftNet,backbone_f=t(depth=18,small_input=False)" "tc.swiftnet_cityscapes_halfres,lr_scheduler_f=lr.QuarterCosLR,epoch_count=${epoch_count},batch_size=8" --params "resnet:backbone->backbone.backbone:resnet18" --no_train_eval
