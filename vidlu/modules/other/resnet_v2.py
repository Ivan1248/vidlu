...
class resnet_v2(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10):
        ...
        assert block in (
            BottleneckV2, BasicBlockV2), "Argument block should be BottleneckV2 or BasicBlockV2"
        self.inplanes = 64
...
def resnet_v2_18(pretrained=False, **kwargs):
    """Constructs a ResNet-18 V2 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = resnet_v2(BasicBlockV2, [2, 2, 2, 2], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    return model


def resnet_v2_34(pretrained=False, **kwargs):
    """Constructs a ResNet-34 V2 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = resnet_v2(BasicBlockV2, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet34']))
    return model


def resnet_v2_50(pretrained=False, **kwargs):
    """Constructs a ResNet-50 V2 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = resnet_v2(BottleneckV2, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet50']))
    return model


def resnet_v2_101(pretrained=False, **kwargs):
    """Constructs a ResNet-101 V2 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = resnet_v2(BottleneckV2, [3, 4, 23, 3], **kwargs)
    if pretrained:
...
def resnet152_v2(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = resnet_v2(BottleneckV2, [3, 8, 36, 3], **kwargs)
    if pretrained:
