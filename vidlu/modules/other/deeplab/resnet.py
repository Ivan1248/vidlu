...
class _ConvBnReLU(nn.Sequential):
    ...
    def __init__(
        ...
        self.add_module("bn", _BATCH_NORM(out_ch, eps=1e-5, momentum=0.01))
...
class _Bottleneck(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, stride, dilation, downsample, checkpoint=False):
...
class _ResLayer(nn.Sequential):
    ...
    def forward(self, *x):
        from vidlu.torch_utils import checkpoint_fix
        return super().forward(*x) if self.checkpoint else checkpoint_fix(super().forward, *x)
