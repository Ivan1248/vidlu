...
class ChannelDeconv(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            cov = x1_s @ x1_s.t() / x1_s.shape[1] + self.eps * torch.eye(B, dtype=x.dtype,
                                                                         device=x.device)
...
class Delinear(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=True, eps=1e-5, n_iter=5, momentum=0.1,
                 block=512):
...
class FastDeconv(conv._ConvNd):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1,
        ...
        super(FastDeconv, self).__init__(
            in_channels, out_channels, _pair(kernel_size), _pair(stride), _pair(padding),
            _pair(dilation),
        ...
        if groups == 1:
            ...
            self.register_buffer('running_deconv',
    ...
    def forward(self, x):
        ...
        if self.training and (not frozen):
            ...
            if self.groups == 1:
                # (C//B*N*pixels,k*k*B)
                X = X.view(-1, self.num_features, C // B).transpose(1, 2).contiguous().view(-1,
                                                                                            self.num_features)
            else:
            ...
            if self.groups == 1:
                ...
                Id = torch.eye(self.num_features, dtype=X.dtype, device=X.device).expand(
                    self.groups, self.num_features, self.num_features)
        ...
        if self.groups == 1:
            w = self.weight.view(-1, self.num_features, C // B).transpose(1, 2).contiguous().view(
                -1, self.num_features) @ deconv
            if self.bias is not None:
            ...
            if self.bias is not None:
                ...
                b = -(w @ (X_mean.view(-1, self.num_features, 1))).view(self.bias.shape)
