...
Label = namedtuple('Label', [
    ...
    # Then create ground truth images with train IDs, using the tools provided
    # in the 'preparation' folder. However, make sure to validate or submit
    # results to our evaluation server using the regular IDs above!
    ...
    'hasInstances',  # Whether this label distinguishes between single instances
    # or not
...
# Note that you might want to ignore labels with ID  -1 during training.
# Further note that the current train IDs are only a suggestion. You can use
# whatever you like. Make sure to provide your results using the original IDs
# and not the training IDs. Note that many IDs are ignored in evaluation and
# thus you never need to predict these!
...
if __name__ == "__main__":
    ...
    for label in labels:
        print("    {:>21} | {:>3} | {:>7} | {:>14} | {:>10} | {:>12} | {:>12}".format(
            label.name, label.id, label.trainId, label.category, label.categoryId,
    ...
    name = 'car'
    id_ = name2label[name].id_
    print("ID of label '{name}': {id}".format(name=name, id=id_))

    # Map from ID to label
    category = id2label[id_].category
    print("Category of label with ID '{id}': {category}".format(id=id_, category=category))
