...
def get_resnet_state_from_checkpoint_file(state_path, include_first_conv=True):
    ...
    param_name_to_array = torch.load(state_path)
    param_name_to_array = {k: v for k, v in param_name_to_array.items() if filter_param_name(k)}
...
def get_densenet_state_from_checkpoint_file(state_path):
    # Checkpoint files can be downloaded here:
...
def get_ladder_densenet_parameters_from_checkpoint_file(checkpoint_file_path):
    # Checkpoint files can be downloaded here:
    # https://github.com/pudae/tensorflow-densenet
    param_name_to_array = get_densenet_state_from_checkpoint_file(
