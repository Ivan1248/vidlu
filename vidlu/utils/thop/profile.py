...
def profile(model, input_size, custom_ops={}, device="cpu"):
    ...
    def add_hooks(m):
        ...
        if m_type in custom_ops:
            ...
            for t in register_hooks:
                if issubclass(m_type, t):
                    fn = register_hooks[t]
                    supertype = True
            else:
        ...
        if fn is not None:
            print(f"Registered FLOP counter for module {m}"
                  + (f" assuming computational equivalence to type {m_type}" if supertype else ""))
